...
object UserDAO {
    ...
    DB.withConnection { implicit c =>
      ...
      val numMatches: Int = if (userName.length <= 0) {
        SQL("""
                    ...
                    | WHERE id={userId};
                """.stripMargin).on(
          "userId" -> user.id).as(SqlParser.int("numMatches").single)
      } else {
        SQL("""
                | SELECT COUNT(*) as numMatches
                | FROM `users`
    ...
    DB.withTransaction { implicit c =>
      ...
      val u: User = SQL(
        """
             ...
             | LIMIT 1;
            """.stripMargin).on(
          "userId" -> user.id).as(userParser.single)
          ...
          if (password.length == 0) {
            ...
            val oldPassword: Option[String] = SQL(
              """
                         ...
                         | LIMIT 1;
                        """.stripMargin).on(
                "userId" -> user.id).as(SqlParser.get[Option[String]]("password").single)
  ...
  private def insertPluralAttributes(tableName: String,
    ...
    SQL("""
      ...
      "value" -> value,
      "type" -> attributeType,
      "primary" -> primary.getOrElse(false)).executeInsert()
  }
  ...
  private def createWithConnection(
    ...
    SQL(
      """
        ...
        "created" -> m.created,
        "lastModified" -> m.lastModified).executeInsert()
      ...
      case Some(addrs) =>
        for (addr <- addrs) {
          SQL("""
            ...
            "primary" -> addr.primary.getOrElse(false)).executeInsert()
        }
